...
process op position mem =
    case op of
        ...
        Print { addr } ->
            let
                loggedValue =
                    Intcode.getParam addr mem

                logEntry =
                    Just
                        { position = Intcode.unwrapParam addr
                        , value = loggedValue
                        }
            in
...
parseOpcode1 : Int -> Memory -> Maybe Op
parseOpcode1 =
    Intcode.parseWith
        [ ( 1
          , Op3
...
parseOpcode2 : Int -> Memory -> Maybe Op
parseOpcode2 =
    Intcode.parseWith
        [ ( 1
          , Op3
...
go1 : Log -> Int -> Memory -> ( Memory, Log )
go1 log position mem =
    case Intcode.step parseOpcode1 process position mem of
        Err err ->
            let
                _ =
                    Debug.log "Error" err
        ...
        Ok ( Just newPosition, maybeLogEntry, newMem ) ->
            ...
            in
            go1 newLog newPosition newMem


go2 : Log -> Int -> Memory -> ( Memory, Log )
go2 log position mem =
    let
        _ =
            Debug.log "go" ( position, mem )
    in
    case Intcode.step parseOpcode2 process position mem of
        Err err ->
            let
                _ =
                    Debug.log "Error" err
