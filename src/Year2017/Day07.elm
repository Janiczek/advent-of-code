...
main : Program Never ( Output, Output2 ) Never
main =
    Advent.program
        { input = input
        , parse1 = parse
...
parseRow : List String -> Thing
parseRow list =
    case list of
        [ name, weightString ] ->
            Leaf
                name
                (Advent.toInt weightString)

        [ name, weightString, _, childrenString ] ->
            Node
                name
                (Advent.toInt weightString)
...
parseChildren : String -> List String
...
compute1 input =
    let
                ...
                |> List.concatMap
                    (\thing ->
                        case thing of
                            ...
                            Node parent weight children ->
                                children
                                    |> List.map (\child -> ( child, parent ))
                    )
                |> List.foldl
                    (\( child, parent ) dict -> Dict.insert child parent dict)
                    Dict.empty
        ...
        root =
            input
                |> List.map name
                |> List.filter (\name -> children |> List.member name |> not)
                |> List.head
                |> Maybe.withDefault "bllaaaaaaaaaaaaaaaah"
    in
        root
        ...
        |> List.map
            (\thing ->
                let
                    ...
                    thingWeight =
                        findWeightRecursive input thingName
                in
                    ( thing
                    , case thing of
                        Leaf _ _ ->
                        ...
                        Node _ _ children ->
                            children
                                |> List.map
                                    (\childName ->
                                        ( childName, findWeightRecursive input childName )
                                    )
                    )
            )
        |> List.filter
...
findWrongChild : Input -> List ( String, Int ) -> Int
findWrongChild input children =
    --_ : (280 ChildrenSum, 300 ShouldHaveBeen, 50 WeightOfChild)
    --_ : (50 + (300 - 280))
    let
        ( childName, childSum ) =
            children
                |> List.filter
                    (\( _, testedSum ) ->
                        (children
                            |> List.filter (\( _, sum ) -> sum == testedSum)
                            |> List.length
                        )
                            == 1
                    )
        ...
        childWeight =
            findWeightOne input childName

        othersSum =
            children
                |> List.filter (\( testedName, _ ) -> testedName /= childName)
                |> List.head
                |> Maybe.map Tuple.second
                |> Maybe.withDefault -1
    in
        --68 + 243 - 251
        childWeight + othersSum - childSum


findWeightOne : Input -> String -> Int
findWeightOne input wantedName =
    input
        |> List.filter (\thing -> name thing == wantedName)
        |> List.map weight
        |> List.head
        |> Maybe.withDefault -1


findWeightRecursive : Input -> String -> Int
findWeightRecursive input wantedName =
    let
        weightOfChildren thing =
            case thing of
                Leaf _ _ ->
                    0
                ...
                Node _ _ children ->
                    children
                        |> List.map (findWeightRecursive input)
                        |> List.sum
    in
        input
            |> List.filter (\thing -> name thing == wantedName)
            |> List.map (\thing -> weight thing + weightOfChildren thing)
            |> List.head
            -- should not happen ↓
            |> Maybe.withDefault -1
