...
isDigitChar : Str -> Bool
isDigitChar = \str ->
    # TODO horribly inefficient, constructing the whole range instead of one if...
    # Need to learn pattern matching
    List.range { start: At 48, end: At 57 }
    |> List.map List.single
    |> List.contains (Str.toScalars str)

firstAndLastNum : List Str -> Nat
firstAndLastNum = \chars ->
    # TODO operator for Str.concat?
    first = List.first chars
                |> Result.withDefault "0"
    last = List.last chars
                |> Result.withDefault "0"
    both = Str.concat first last
    when Str.toNat both is
        Err InvalidNumStr -> 0
        Ok n -> n
...
testInput1 =
  [
    # TODO later: figure out multiline strings
    # TODO later: figure out reading from files
    "1abc2",
    "pqr3stu8vwx",
    "a1b2c3d4e5f",
    "treb7uchet",
