...
let input: string = "4 * 3 * (2 * 6 + (3 * 3 + 8 + 7 + 2) + 9 + 8) * 4 * 6
...
let opp1 = new OperatorPrecedenceParser<Expr,unit,unit>()
let expr1 = opp1.ExpressionParser
let term1 =
    (between (pstring "(" >>. spaces) (pstring ")" >>. spaces) expr1 |>> Parenthesized)
    <|>
    (pint64 .>> spaces |>> Int)
opp1.TermParser <- term1
opp1.AddOperator(InfixOperator("+",spaces,1,Associativity.Left,fun x y -> Add(x,y)))
opp1.AddOperator(InfixOperator("*",spaces,1,Associativity.Left,fun x y -> Multiply(x,y)))

let parseExpr1 (input: string): Expr =
    match run expr1 input with
    | Success(result,_,_) -> result
    | Failure(errorMsg,_,_) -> failwithf "error parsing: %A" errorMsg
    
let opp2 = new OperatorPrecedenceParser<Expr,unit,unit>()
let expr2 = opp2.ExpressionParser
let term2 =
    (between (pstring "(" >>. spaces) (pstring ")" >>. spaces) expr2 |>> Parenthesized)
    <|>
    (pint64 .>> spaces |>> Int)
opp2.TermParser <- term2
opp2.AddOperator(InfixOperator("+",spaces,2,Associativity.Left,fun x y -> Add(x,y)))
opp2.AddOperator(InfixOperator("*",spaces,1,Associativity.Left,fun x y -> Multiply(x,y)))

let parseExpr2 (input: string): Expr =
    match run expr2 input with
    | Success(result,_,_) -> result
    | Failure(errorMsg,_,_) -> failwithf "error parsing: %A" errorMsg

let rec eval (expr: Expr): int64 =
    match expr with
    | Int int -> int
    | Parenthesized expr_ -> eval expr_
    | Add (a,b) -> eval a + eval b
    | Multiply (a,b) -> eval a * eval b
    
let parse1 (input: string): Expr [] =
    input.Split "\n"
    |> Array.map parseExpr1
    
let parse2 (input: string): Expr [] =
    input.Split "\n"
    |> Array.map parseExpr2

let part1 (input: Expr []): int64 =
    input
    |> Array.map eval
    |> Array.sum
    
let part2 (input: Expr []): int64 =
    input
    |> Array.map eval
    |> Array.sum
