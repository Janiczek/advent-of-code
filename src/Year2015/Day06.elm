module Year2015.Day06 exposing (Input1, Input2, Instruction(..), Output1, Output2, Rectangle, compute1, compute2, input_, main, parse1, parse2, parseLine, parseRectangle, processInstruction, tests1, tests2)
...
parseLine string =
    case String.words string of
        "turn" :: "on" :: rest ->
            TurnOn (parseRectangle rest)

        "turn" :: "off" :: rest ->
            TurnOff (parseRectangle rest)

        "toggle" :: rest ->
            Toggle (parseRectangle rest)

        _ ->
            Debug.todo <| "Wrong input " ++ string


parseRectangle : List String -> Rectangle
parseRectangle words =
    let
        ( ( left, top ), ( right, bottom ) ) =
            case words of
                [ a, "through", b ] ->
                    ( parsePartial a, parsePartial b )

                _ ->
                    Debug.todo <| "Wrong input " ++ Debug.toString words

        parsePartial partial =
            case String.split "," partial of
                [ a, b ] ->
                    ( unsafeToInt a, unsafeToInt b )

                _ ->
                    Debug.todo <| "Wrong input " ++ partial
    in
...
map : (a -> a) -> Rectangle -> Grid a -> Grid a
map fn { left, top, right, bottom } grid =
    grid
        |> Array.indexedMap
            (\rowIndex row ->
                if top <= rowIndex && rowIndex <= bottom then
                    row
                        |> Array.indexedMap
                            (\colIndex cell ->
                                if left <= colIndex && colIndex <= right then
                                    fn cell
...
toggle : Rectangle -> Grid Bool -> Grid Bool
toggle =
    map not


turnOn : Rectangle -> Grid Bool -> Grid Bool
turnOn =
    map (always True)


turnOff : Rectangle -> Grid Bool -> Grid Bool
turnOff =
    map (always False)


toggle2 : Rectangle -> Grid Int -> Grid Int
toggle2 =
    map ((+) 2)


turnOn2 : Rectangle -> Grid Int -> Grid Int
turnOn2 =
    map ((+) 1)


turnOff2 : Rectangle -> Grid Int -> Grid Int
turnOff2 =
    map (\x -> max 0 (x - 1))
...
empty : Grid Bool
empty =
    Array.repeat 1000 (Array.repeat 1000 False)
...
compute1 input =
    input
        |> List.foldl processInstruction empty
        |> countLit
            (\bool ->
                if bool then
                    1
...
processInstruction : Instruction -> Grid Bool -> Grid Bool
processInstruction instruction grid =
    case instruction of
        TurnOn rectangle ->
            turnOn rectangle grid

        TurnOff rectangle ->
            turnOff rectangle grid

        Toggle rectangle ->
            toggle rectangle grid


countLit : (a -> Int) -> Grid a -> Int
countLit fn grid =
    let
        countLitInRow : Array a -> Int
        countLitInRow row =
            Array.foldl (\cell acc -> acc + fn cell) 0 row
    in
    Array.foldl (\row acc -> acc + countLitInRow row) 0 grid


compute2 : Input2 -> Output2
compute2 input =
    input
        |> List.foldl processInstruction2 empty2
        |> countLit identity


processInstruction2 : Instruction -> Grid Int -> Grid Int
processInstruction2 instruction grid =
    case instruction of
        TurnOn rectangle ->
            turnOn2 rectangle grid

        TurnOff rectangle ->
            turnOff2 rectangle grid

        Toggle rectangle ->
            toggle2 rectangle grid
