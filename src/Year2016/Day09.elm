...
parserHelp vs =
    P.oneOf
        [ anything
            |> P.map (\v -> Loop (v :: vs))
        , P.succeed ()
            |> P.map (\_ -> Done (String.concat (List.reverse vs)))
        ]
        ...
        |> P.andThen
            (\( chunkLength, times ) ->
                P.loop
                    ...
                    nChars
                    |> P.map (String.repeat times)
            )


type alias LoopState =
    { todo : Int
    , soFarReversed : List String
    }


nChars : LoopState -> Parser (Step LoopState String)
nChars { todo, soFarReversed } =
    if todo == 0 then
        soFarReversed
            |> List.reverse
            |> String.concat
        ...
        P.chompIf (always True)
            ...
            |> P.map
                (\charStr ->
                    P.Loop
                        { todo = todo - 1
                        , soFarReversed = charStr :: soFarReversed
                        }
                )


decompress : String -> Result (List P.DeadEnd) String
decompress input =
    P.run parser input
