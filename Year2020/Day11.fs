...
let realInput = "LLLLLL.LLLL..LLLLLL.LLLLLLLLLLLLLLLLLLLLL.LLLLLL.LLLLLLLL.LLLLL.LLLLLL.L.LL.LLLLLL.LLLLLLLLLLLLLLLL
...
let parse (input: string): Map<int * int, char> =
    input.Split "\n"
    |> Array.indexed
    |> Array.toList
    |> List.map (fun (row, line) ->
        ...
        |> Array.indexed
        |> Array.toList
        |> List.map (fun (col, char) -> ((row, col), char)))
    |> List.concat
    |> Map.ofList

let neighbours1 (row, col) (map: Map<int * int, char>): int =
    [ (row - 1, col - 1)
      (row - 1, col)
      (row - 1, col + 1)
      (row, col - 1)
    ...
    |> List.choose (fun coord ->
        map.TryFind(coord)
        |> Option.filter (fun char -> char = '#'))
    |> List.length

let step1 (map: Map<int * int, char>): Map<int * int, char> =
    map
    |> Map.map (fun coord seat ->
        let n = neighbours1 coord map
        if seat = 'L' && n = 0 then '#'
        else if seat = '#' && n >= 4 then 'L'
        else seat)

let rec fix (fn: 'a -> 'a) (x: 'a): 'a =
    let next = fn x
    if next = x then x else fix fn next

let part1 (map: Map<int * int, char>): int =
    map
    |> fix step1
    |> Map.filter (fun _ c -> c = '#')
    |> Map.count

let neighbours2 (row, col) (map: Map<int * int, char>): int =
    let rec findInDirection (dR, dC) (r, c): bool =
        let (rr,cc) = (r + dR, c + dC)
        match map.TryFind((rr, cc)) with
        | None -> false
        | Some '#' -> true
        | Some 'L' -> false
        | Some '.' -> findInDirection (dR, dC) (rr,cc)
        | Some _ -> failwith "wut"
    ...
    [ (-1, -1)
      ...
      (1, 0)
      (1, 1) ]
      |> List.filter (fun d -> findInDirection d (row, col))
      |> List.length

let step2 (map: Map<int * int, char>): Map<int * int, char> =
    map
    |> Map.map (fun coord seat ->
        let n = neighbours2 coord map
        if seat = 'L' && n = 0 then '#'
        else if seat = '#' && n >= 5 then 'L'
        else seat)

let part2 (map: Map<int * int, char>): int =
    map
    |> fix step2
    |> Map.filter (fun _ c -> c = '#')
    |> Map.count

let printMap (map: Map<int * int, char>): unit =
    map
    |> Map.toList
    |> List.sortBy fst
    |> List.groupBy (fst << fst)
    |> List.map snd
    |> List.map (List.map snd >> List.toArray)
    |> List.toArray
    |> Array.map System.String
    |> fun strings -> System.String.Join("\n", strings)
    |> printfn "%s\n"

let main =
    realInput |> parse |> part2
