module Year2016.Day07 exposing (..)
...
supportsTls ip =
    let
        ...
        outsideGroups =
            allGroups
                |> List.Extra.groupsOfWithStep 1 2
                |> List.concat
        ...
        insideGroups =
            allGroups
                ...
                |> List.concat
    in
        (List.any containsAbba outsideGroups)
            && (not (List.any containsAbba insideGroups))


containsAbba : List Char -> Bool
containsAbba list =
    case list of
        a :: b :: c :: d :: rest ->
            ((a == d) && (b == c) && (a /= b))
                || (containsAbba (List.drop 1 list))
...
supportsSsl ip =
    ...
    in
        abs
            |> List.any (\x -> List.member x bas)


findABs : List Char -> List ( Char, Char )
findABs chars =
    case chars of
        a :: b :: c :: rest ->
            if a /= b && a == c then
                ( a, b ) :: findABs (List.drop 1 chars)
            else
                findABs (List.drop 1 chars)
...
tests1 =
    [ Test "1" "abba[mnop]qrst" [ "abba[mnop]qrst" ] 1
    , Test "2" "abcd[bddb]xyyx" [ "abcd[bddb]xyyx" ] 0
    , Test "3" "aaaa[qwer]tyui" [ "aaaa[qwer]tyui" ] 0
    , Test "4" "ioxxoj[asdfgh]zxcvbn" [ "ioxxoj[asdfgh]zxcvbn" ] 1
    ]


tests2 : List (Test Input Output)
tests2 =
    [ Test "1" "aba[bab]xyz" [ "aba[bab]xyz" ] 1
    , Test "2" "xyx[xyx]xyx" [ "xyx[xyx]xyx" ] 0
    , Test "3" "aaa[kek]eke" [ "aaa[kek]eke" ] 1
    , Test "4" "zazbz[bzb]cdb" [ "zazbz[bzb]cdb" ] 1
    ]
