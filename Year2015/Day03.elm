...
compute1 input =
    input
        |> List.foldl processStep1
            { current = startingPos
            , houses = Dict.empty |> givePresent startingPos
            }
        |> .houses
        |> Dict.size


givePresent : Pos -> Dict Pos Int -> Dict Pos Int
givePresent pos houses =
    houses
        |> Dict.update pos (Maybe.map ((+) 1) >> Maybe.withDefault 1 >> Just)


type alias Intermediate1 =
    { current : Pos
    , houses : Dict Pos Int
    }


processStep1 : Step -> Intermediate1 -> Intermediate1
processStep1 step { current, houses } =
    let
        ( x, y ) =
            current
...
processStep2 : Step -> Intermediate2 -> Intermediate2
processStep2 step { santa, robot, turn, houses } =
    let
        ( x, y ) =
            case turn of
                Santa ->
