...
parseSue string =
    ...
    of
        _ :: id :: rest ->
            ...
            in
                { id = Advent.toInt id
                , children = pairs |> Dict.get "children"
                , cats = pairs |> Dict.get "cats"
                , samoyeds = pairs |> Dict.get "samoyeds"
                , pomeranians = pairs |> Dict.get "pomeranians"
                , akitas = pairs |> Dict.get "akitas"
                , vizslas = pairs |> Dict.get "vizslas"
                , goldfish = pairs |> Dict.get "goldfish"
                , trees = pairs |> Dict.get "trees"
                , cars = pairs |> Dict.get "cars"
                , perfumes = pairs |> Dict.get "perfumes"
                }
...
compute1 input =
    input
        |> List.filter
            (\sue ->
                (sue.children == needed.children || sue.children == Nothing)
                    && (sue.cats == needed.cats || sue.cats == Nothing)
                    && (sue.samoyeds == needed.samoyeds || sue.samoyeds == Nothing)
                    && (sue.pomeranians == needed.pomeranians || sue.pomeranians == Nothing)
                    && (sue.akitas == needed.akitas || sue.akitas == Nothing)
                    && (sue.vizslas == needed.vizslas || sue.vizslas == Nothing)
                    && (sue.goldfish == needed.goldfish || sue.goldfish == Nothing)
                    && (sue.trees == needed.trees || sue.trees == Nothing)
                    && (sue.cars == needed.cars || sue.cars == Nothing)
                    && (sue.perfumes == needed.perfumes || sue.perfumes == Nothing)
            )
        |> List.head
        |> Advent.unsafeMaybe
        |> .id
...
compute2 input =
    input
        |> List.filter
            (\sue ->
                (sue.children == needed.children || sue.children == Nothing)
                    && (Maybe.map2 (>) sue.cats needed.cats |> Maybe.withDefault True)
                    && (sue.samoyeds == needed.samoyeds || sue.samoyeds == Nothing)
                    && (Maybe.map2 (<) sue.pomeranians needed.pomeranians |> Maybe.withDefault True)
                    && (sue.akitas == needed.akitas || sue.akitas == Nothing)
                    && (sue.vizslas == needed.vizslas || sue.vizslas == Nothing)
                    && (Maybe.map2 (<) sue.goldfish needed.goldfish |> Maybe.withDefault True)
                    && (Maybe.map2 (>) sue.trees needed.trees |> Maybe.withDefault True)
                    && (sue.cars == needed.cars || sue.cars == Nothing)
                    && (sue.perfumes == needed.perfumes || sue.perfumes == Nothing)
            )
        |> List.head
